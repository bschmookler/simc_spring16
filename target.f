...
10	continue
	...
	if (liquid) then			!liquid target
	  ...
	  else if (targ%can .eq. 3) then	!GMP Target (entrance window, 0.147 mm Al)
	    s_Al = s_Al + 0.0147
	  endif
	endif
...
! ... For GMP Cryo Target:
! ... width: 2.630", wall thickness: 0.203 mm Al
! ... the target has a tip of radius 1.315" and thickness 0.147 mm Al
! ... Although any target length can be set in input file, it's really 15 cm 

! ... compute distances travelled for Spring 2016 LHRS(RHRS)
...
! ........ Again I'm assuming theta < 90 degrees
	   if (z%max.ge.targ%length/2.) then !This should always be true
	      ...
	      th_corner_max = atan(1.315*inch_cm/(targ%length/2.-z%max))
	   endif
! angle to start of end-cap
	   th_corner_min = atan( (1.315*inch_cm)/(targ%length -(1.315*inch_cm)) )
