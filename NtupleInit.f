	...
	if (doing_pion .or. doing_kaon .or. doing_delta) then
	  ...
	  NtupleTag(m) = 'PmOop'	! 40
	  m = m+1
	  NtupleTag(m) = 'fry'		! 41		!+y is up (fry points down).
	  m = m+1
	  NtupleTag(m) = 'frx'		! 42		!+x is right (frx points left).
	  m = m+1
	  NtupleTag(m) = 'radphot'	! 43
	  m = m+1
	  NtupleTag(m) = 'sigcc'	! 44
	  m = m+1
	  NtupleTag(m) = 'Weight'	! 45
	  m = m+1
	  NtupleTag(m) = 'success'      ! 46
	  m = m+1
	  NtupleTag(m) = 'stopwhen'     ! 47
	  m = m+1
	  NtupleTag(m) = 'x_stop'       ! 48
	  m = m+1
	  NtupleTag(m) = 'y_stop'       ! 49
	  m = m+1
	  NtupleTag(m) = 'eilossi'      ! 50
	  m = m+1
	  NtupleTag(m) = 'eflossi'      ! 51
	  m = m+1
	  NtupleTag(m) = 'hflossi'      ! 52
	  m = m+1
	  NtupleTag(m) = 'eiloss'       ! 53
	  m = m+1
	  NtupleTag(m) = 'efloss'       ! 54
	  m = m+1
	  NtupleTag(m) = 'hfloss'       ! 55
	  m = m+1
	  NtupleTag(m) = 'zreacti'      ! 56
	  m = m+1
	  NtupleTag(m) = 'ezreact'      ! 57
	  m = m+1
	  NtupleTag(m) = 'pzreact'      ! 58
	  m = m+1
	  NtupleTag(m) = 'eytari'       ! 59
	  m = m+1
	  NtupleTag(m) = 'pytari'       ! 60
	  m = m+1
	  NtupleTag(m) = 'edeltai'      ! 61
	  m = m+1
	  NtupleTag(m) = 'pdeltai'      ! 62
	endif
